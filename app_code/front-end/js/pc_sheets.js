...
function saveFormValues() {
  ...
  async function generateCommand(playerNum) {
    ...
    const response = await window.electron.sendCommand(payload);
  ...
  // Generate and display image
    function updateImage(playerData, playerNum) {
        ...
        const imageUrl = `../${playerNum}.png?t=${timestamp}`;
...
async function display(playerNum){
    let command = "python3 eink_screen_upload.py " // output.jpg 1
    const playerData = getPlayerValues(playerNum);
    //console.log(playerData);
    command += "./" + playerNum + ".png " + playerData["eink_selector"];
    console.log(command);
    ...
    document.getElementById(progressUpload).textContent = reponse;
}
...
async function BTconnectScreen(screen) {
  ...
  const terminalCommand = `gnome-terminal -- bash -c "echo Mecha/200/ | sudo -S  rfcomm connect /dev/rfcomm${screen} ${mac_adds[screen]} 1 & read; exec bash"`; // For GNOME Terminal
  //command = "sudo rfcomm connect /dev/rfcomm" + screen + "0 C8:F0:9E:B4:1C:CE 1 &";
  reponse = await window.electron.sendCommand(terminalCommand);
  console.log(reponse);
}
